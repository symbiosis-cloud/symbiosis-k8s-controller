...
type CertificateSigningRequestReconciler struct {
	...
	client.Client
	SymbiosisClient      symbiosis.Client
	ClusterID            string
...
func (r *CertificateSigningRequestReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, returnErr error) {
	...
	if len(x509cr.DNSNames)+len(x509cr.IPAddresses) == 0 {
		...
		appendCondition(&csr, false, reason)
	} else if valid, reason := r.ProviderChecks(ctx, &csr, x509cr); !valid {
		l.V(0).Info("CSR request did not pass the symbiosis-specific tests. Reason: " + reason)
		appendCondition(&csr, false, reason)
	} else {
